...
def _upsell_suggestions(user: dict) -> dict:
    ...
    if is_subscriber:
        recommended = "refill_5" if balance == 0 else "subscriber_refills"
        message = (
            "Out of credits? Grab a Quick Refill — ~40% cheaper than bundles."
            if balance == 0
            else "Running low — subscriber refills are ~40% cheaper than credit bundles."
        )
    else:
        recommended = "sub_pro" if balance == 0 else "subscriptions"
        message = (
            "Subscribe to Pro for the best monthly value, or buy a credit bundle to keep testing."
            if balance == 0
            else "Subscribe to unlock ~40% cheaper refills when your credits run out."
        )
    return {
